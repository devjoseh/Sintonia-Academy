...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (lennys.length === 0) {
      return message.channel.send('Desculpe, não há cantadas disponíveis no momento.');
    }

    const rand_num = Math.floor(Math.random() * lennys.length);
    const rand_val = lennys[rand_num];

    if (!ss.titulo || !ss.footer) {
      return message.channel.send('Desculpe, não foi possível encontrar as configurações necessárias.');
    }
